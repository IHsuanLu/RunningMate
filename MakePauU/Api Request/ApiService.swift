...
class ApiService: NSObject{
    ...
    func updateUser(userInfo: UserInfo, completion: @escaping () -> ()){
        
        let circleURL = URL(string: "http://140.119.19.149:8000/update_member/")
        
        let session = URLSession.shared
        var request = URLRequest(url: circleURL!)
        
        
        let testString = "member_id=\(MemberId.sharedInstance.member_id)&name=\(userInfo.name!)&sex=\(userInfo.sex!)&居住地=\(userInfo.living!)&school=\(userInfo.school!)&sex_prefer=\(userInfo.sex_prefer!)&興趣=\(userInfo.interest!)&困擾=\(userInfo.problem!)&嘗試=\(userInfo.tries!)"
